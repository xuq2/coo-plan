    ...
    async handleFormSubmit(year) {
      ...
      if (goalName && Object.keys(goalTime).length !== 0) {
        try {
          ...
          fetch(
            "https://ij5p8quwsi.execute-api.us-west-2.amazonaws.com/dev/user/addgoals",
            requestOptions
          )
            .then((response) => {
  ...
  return (
    <Form form={form} layout="vertical">
          ...
          {goalTasks.map((task, index) => (
            <div style={{ marginBottom: 5, marginTop: 5 }} key={index}>
              <Checkbox
                ...
                onChange={(e) => updateTaskToggle(e, index)}
              >
                <Input
                  value={task.msg}
      ...
      <Form.Item label="Select your terms">
        ...
        >
          <Checkbox.Group
            ...
            onChange={(checkedValues) => setGoalTerms(checkedValues)}
          >
            <Checkbox value="fall">
              <Tag color="orange">Fall</Tag>
            </Checkbox>
            <Divider type="vertical" />
            <Checkbox value="winter">
              <Tag color="blue">Winter</Tag>
            </Checkbox>
            <Divider type="vertical" />
            <Checkbox value="spring">
              <Tag color="green">Spring</Tag>
            </Checkbox>
            <Divider type="vertical" />
            <Checkbox value="summer">
      ...
      <Form.Item label="Time">
        <RangePicker
          value={
            Object.keys(goalTime).length === 0
              ? []
              : [moment(goalTime.start_time), moment(goalTime.end_time)]
          }
